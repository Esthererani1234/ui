  ...
  return (
    <article className="product-card">
      ...
      <Link className="product-image" to={`/product/${product.slug}`}>
        {product.image_url ? <img src={product.image_url} alt={product.name} /> : <div className={`bullion-art ${product.metal} ${product.category}`}><span>{metalSymbol(product.metal)}</span><small>{product.metal_weight_oz} TROY OZ</small></div>}
      </Link>
