  ...
  return <section className="section account-section"><div className="container">
    ...
    <div className="account-layout"><aside className="account-nav">{tabs.map(([id, Icon, label]) => <button key={id} className={active === id ? "active" : ""} onClick={() => selectTab(id)}><Icon /> {label}</button>)}</aside>
      <div className="account-content">
        ...
        {active === "security" && <div><div className="account-panel-heading"><div><h2>Account security</h2><p>Protect access to order and delivery information.</p></div><ShieldCheck /></div><div className="security-card"><LockKeyhole /><div><b>Password</b><p>Use a unique password with at least 12 characters. A reset link is sent only to your verified email.</p><button className="button button-dark" onClick={sendPasswordReset} disabled={busy}>{busy ? "Sending…" : "Send password-reset link"}</button></div></div><div className="security-facts"><div><span>Email verification</span><b>{user.email_confirmed_at ? "Verified" : "Confirmation required"}</b></div><div><span>SMS identity protection</span><b>{phoneMfaVerified ? "Verified" : requiresCustomerMfa ? "Required" : "Rollout pending"}</b></div><div><span>Last sign-in</span><b>{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleString() : "Not available"}</b></div><div><span>Account identifier</span><b>{user.id.slice(0, 8).toUpperCase()}</b></div></div>{requiresCustomerMfa && !phoneMfaVerified && <Link className="button button-gold" to="/verify-phone?return=/account?tab=security">Set up SMS security</Link>}{message && <div className="form-message">{message}</div>}<div className="security-warning"><b>GoldOnTheSpot will never ask for your password or authenticator code.</b><span>Do not send card numbers, banking passwords, or verification codes through chat, email, or support tickets.</span></div></div>}
