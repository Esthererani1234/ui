...
export default function AuthPage() {
  ...
  const [form, setForm] = useState({ email: "", password: "", confirmPassword: "", firstName: "", lastName: "", agree: false });
  ...
  const submit = async (event) => {
    ...
    const result = mode === "signup"
      ? await supabase.auth.signUp({ email: form.email.trim(), password: form.password, options: { data: { first_name: form.firstName.trim(), last_name: form.lastName.trim() }, emailRedirectTo: `${window.location.origin}${destination}` } })
    ...
    else if (mode === "signup" && !result.data.session) { setMessageType("success"); setMessage("Check your email and select Confirm account. Then return here to sign in."); }
  };
