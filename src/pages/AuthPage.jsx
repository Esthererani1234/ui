...
import { authDestination, isEmailSendRateLimit, saveVerifiedSignup } from "../lib/signupCompletion";
...
export default function AuthPage() {
  ...
  const destination = authDestination(params.get("return"));
  const signupDestination = authDestination(params.get("return"), "/");
  ...
  if (user && !loading && mode !== "recovery" && !completingSignup) return <Navigate to={destination} replace />;
  ...
  const submitCredentials = async (event) => {
    ...
    if (error) {
      if (isEmailSendRateLimit(error)) {
        ...
        return setMessage("Email sending is temporarily limited. If you already received a code, enter it here; otherwise wait before trying again.");
      }
  ...
  const verifyEmail = async (event) => {
    ...
    let emailVerified = Boolean(verifiedUserId);
    try {
      ...
      await saveVerifiedSignup(supabase, { userId, password: form.password, details: latestDetails });
      ...
      setMessage(emailVerified
        ? `${friendlyError(error)} Your email is already verified. Select Save details & continue to retry.`
        : friendlyError(error));
