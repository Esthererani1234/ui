...
export default function AuthPage() {
  ...
  const submit = async (event) => {
    ...
    const normalizedPhone = toUsE164(form.phone);
    if (mode === "signup" && !normalizedPhone) return setMessage("Enter a valid 10-digit U.S. mobile number.");
  ...
  return <section className="auth-section"><div className="container auth-grid">
    ...
    <div className="auth-card">
      ...
      : <form onSubmit={submit}>{mode === "signup" && <div className="form-row"><label>First name<input required maxLength="60" autoComplete="given-name" value={form.firstName} onChange={(event) => setForm({ ...form, firstName: event.target.value })} /></label><label>Last name<input required maxLength="60" autoComplete="family-name" value={form.lastName} onChange={(event) => setForm({ ...form, lastName: event.target.value })} /></label></div>}<label>Email address<input required type="email" autoComplete="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label>{mode === "signup" && <><label>Mobile number<span className="phone-input"><span aria-hidden="true">+1</span><input required type="tel" inputMode="tel" autoComplete="tel-national" placeholder="(212) 555-0123" maxLength="14" value={form.phone} onChange={(event) => setForm({ ...form, phone: formatUsPhone(event.target.value) })} /></span><small className="field-help">U.S. mobile number for SMS identity verification.</small></label><SmsConsentDisclosure /></>}<PasswordField label="Password" value={form.password} show={showPassword} onShow={() => setShowPassword((value) => !value)} onChange={(value) => setForm({ ...form, password: value })} autoComplete={mode === "signup" ? "new-password" : "current-password"} minLength={mode === "signup" ? 12 : 8} />{mode === "signup" && <><PasswordField label="Confirm password" value={form.confirmPassword} show={showPassword} onShow={() => setShowPassword((value) => !value)} onChange={(value) => setForm({ ...form, confirmPassword: value })} autoComplete="new-password" minLength={12} /><label className="auth-agreement"><input type="checkbox" checked={form.agree} onChange={(event) => setForm({ ...form, agree: event.target.checked })} /> <span>I agree to the <Link to="/terms">Terms &amp; Conditions</Link> and <Link to="/privacy">Privacy Policy</Link>.</span></label></>}{message && <div className={`form-message ${messageType || "error"}`}>{message}</div>}<button className="button button-gold full" disabled={busy}>{busy ? "Please wait…" : mode === "signup" ? "Create secure account" : "Sign in securely"}</button>{mode === "signin" && <button type="button" className="text-button centered" onClick={() => setMode("forgot")}>Forgot password?</button>}</form>}
    </div>
  </div></section>;
}
