...
export default function CheckoutPage() {
  ...
  return <section className="section checkout-section"><div className="container checkout-grid">
    ...
    <aside className="checkout-summary"><h2>Order review</h2><span>{estimatedUnits} item{estimatedUnits === 1 ? "" : "s"}</span>{items.map((item) => { const price = productPrice(item.product, spot); const image = item.product.image_url || item.product.image_urls?.[0]; return <div className="checkout-item" key={item.product.id}><div className={`cart-thumb small ${image ? "has-image" : item.product.metal}`}>{image ? <img src={image} alt={item.product.name} /> : metalSymbol(item.product.metal)}</div><span><b>{item.product.name}</b><small>{item.quantity} × {price == null ? "Loading live price…" : money(price)}</small></span><strong>{price == null ? "—" : money(price * item.quantity)}</strong></div>; })}<div className="checkout-totals"><div><span>Live-priced items</span><b>{spot ? money(estimate.subtotal) : "Loading…"}</b></div><div><span>Insured shipping</span><b>{estimate.shipping ? money(estimate.shipping) : "Free"}</b></div>{estimate.surcharge > 0 && <div><span>Card surcharge</span><b>{money(estimate.surcharge)}</b></div>}<div><span>Estimated total</span><strong>{spot ? money(estimate.total) : "—"}</strong></div></div><div className="checkout-notice"><CheckCircle2 /><span><b>Final price protection</b><small>The server—not your browser—recalculates every line at submission. The confirmation shows the exact locked total.</small></span></div></aside>
  </div></section>;
}
