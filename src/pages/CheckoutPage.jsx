...
export default function CheckoutPage() {
  ...
  const [paymentConfig, setPaymentConfig] = useState({ enabled: false, wire: true, card: false, crypto: false });
  ...
  useEffect(() => {
    fetch("/api/payments/config").then((response) => response.ok ? response.json() : null).then((data) => {
      if (data?.methods) setPaymentConfig({ enabled: Boolean(data.enabled), ...data.methods, wire: data.enabled ? Boolean(data.methods.wire) : true });
    }).catch(() => {});
  }, []);
  ...
  const submit = async (event) => {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke("create-order", { body: { cart: items.map((item) => ({ product_id: item.product.id, quantity: item.quantity })), contact: { first_name: form.firstName.trim(), last_name: form.lastName.trim(), email: user.email, phone: form.phone.trim() }, shipping: { address_line_1: form.address1.trim(), address_line_2: form.address2.trim(), city: form.city.trim(), state: form.state.trim().toUpperCase(), postal_code: form.postalCode.trim(), country: "US" }, payment_method: form.paymentMethod, notes: form.notes.trim(), terms: { accepted: true, version: "2026-08-04" } } });
      if (invokeError || data?.error) throw new Error(data?.error || invokeError?.message || "Unable to place order.");
      let payment = {};
      if (paymentConfig.enabled) {
        const { data: session } = await supabase.auth.getSession();
        const paymentResponse = await fetch("/api/payments/create-checkout", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${session.session?.access_token || ""}` }, body: JSON.stringify({ order_id: data.order_id }) });
        payment = await paymentResponse.json().catch(() => ({}));
        if (!paymentResponse.ok) throw new Error(payment.error || "The order was saved, but payment setup needs attention. Open Orders in your account to retry.");
      }
      if (form.saveAddress) {
  ...
  return <section className="section checkout-section"><div className="container checkout-grid">
    <form className="checkout-form" onSubmit={submit}><div className="checkout-title"><span className="eyebrow dark">SECURE CHECKOUT</span><h1>Delivery and payment</h1><p>Review your verified contact information and insured-delivery address.</p></div>
      ...
      <fieldset><legend>Payment method</legend><div className="payment-options"><PaymentOption value="wire" form={form} setForm={setForm} disabled={!paymentConfig.wire} title="Bank wire" detail={paymentConfig.enabled ? "Encrypted instructions by email and in your order • no surcharge" : "Instructions provided after order review • no surcharge"} /><PaymentOption value="card" form={form} setForm={setForm} disabled={!paymentConfig.card} title="Credit or debit card" detail={paymentConfig.card ? `Secure Stripe Checkout • ${settings.card_surcharge_percent}% processing surcharge` : "Available after processor approval"} /><PaymentOption value="crypto" form={form} setForm={setForm} disabled={!paymentConfig.crypto} title="Crypto" detail={paymentConfig.crypto ? "Choose a supported cryptocurrency on secure hosted checkout • automatic confirmation" : "Available after merchant account setup"} /></div></fieldset>
      <label>Order notes <span>(optional)</span><textarea rows="3" maxLength="1000" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Delivery or product notes" /></label><label className="checkout-check terms-check"><input type="checkbox" checked={form.agree} onChange={(event) => setForm({ ...form, agree: event.target.checked })} /> I agree to the <Link to="/terms">Terms of Purchase</Link>. I understand that submitting this bullion order creates a binding purchase obligation at the displayed locked price, subject to GoldOnTheSpot acceptance and the stated payment deadline.</label>{!settings.accepting_orders && <div className="form-message error">Checkout is temporarily paused. Your cart will stay saved.</div>}{error && <div className="form-message error">{error}</div>}<button className="button button-gold full large" disabled={busy || !spot || !cartReady || !settings.accepting_orders}>{busy ? "Locking price and opening payment…" : !settings.accepting_orders ? "Checkout temporarily paused" : !cartReady ? "Refreshing inventory…" : !spot ? "Loading live prices…" : "Agree & place secure order"}</button><p className="secure-submit"><LockKeyhole /> Checkout recalculates prices and inventory on the server. Card and crypto details are entered only on the payment provider's hosted page.</p>
    </form>
    <aside className="checkout-summary"><h2>Order review</h2><span>{estimatedUnits} item{estimatedUnits === 1 ? "" : "s"}</span>{items.map((item) => { const price = productPrice(item.product, spot); const image = item.product.image_url || item.product.image_urls?.[0]; return <div className="checkout-item" key={item.product.id}><div className={`cart-thumb small ${image ? "has-image" : item.product.metal}`}>{image ? <img src={productImageUrl(image, 140, 78)} alt={item.product.name} width="140" height="140" loading="lazy" decoding="async" /> : metalSymbol(item.product.metal)}</div><span><b>{item.product.name}</b><small>{item.quantity} × {price == null ? "Loading live price…" : money(price)}</small></span><strong>{price == null ? "—" : money(price * item.quantity)}</strong></div>; })}<div className="checkout-totals"><div><span>Live-priced items</span><b>{spot ? money(estimate.subtotal) : "Loading…"}</b></div><div><span>Insured shipping</span><b>{estimate.shipping ? money(estimate.shipping) : "Free"}</b></div>{estimate.surcharge > 0 && <div><span>Card surcharge</span><b>{money(estimate.surcharge)}</b></div>}<div><span>Estimated total</span><strong>{spot ? money(estimate.total) : "—"}</strong></div></div><div className="checkout-notice"><CheckCircle2 /><span><b>Final price protection</b><small>The server—not your browser—recalculates every line at submission. The confirmation shows the exact locked total.</small></span></div></aside>
  </div></section>;
}

function PaymentOption({ value, form, setForm, title, detail, disabled = false }) {
  return <label className={`${form.paymentMethod === value ? "selected" : ""} ${disabled ? "disabled" : ""}`}><input disabled={disabled} type="radio" name="payment" value={value} checked={form.paymentMethod === value} onChange={(event) => setForm({ ...form, paymentMethod: event.target.value })} /><span><b>{title}</b><small>{detail}</small></span></label>;
}
