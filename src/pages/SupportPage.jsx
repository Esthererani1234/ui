...
import { ChevronDown, Headphones, LockKeyhole, MessageSquareText, Send, ShieldCheck, XCircle } from "lucide-react";
...
export default function SupportPage() {
  ...
  return <>
    ...
    {user && <section className="section ticket-history-section"><div className="container"><div className="section-heading"><div><span className="eyebrow dark">SUPPORT INBOX</span><h2>Your conversations</h2><p>Open a ticket to read the full conversation and reply. Replying to an answered ticket reopens it automatically.</p></div><span className="status-pill">{tickets.filter((item) => !["resolved", "closed"].includes(item.status)).length} active</span></div>{notice && <div className="form-message">{notice}</div>}{loading ? <div className="catalog-loading">Loading your private conversations…</div> : tickets.length ? <div className="support-inbox">{tickets.map((item) => { const replyReopens = ["resolved", "closed"].includes(item.status); return <article className={`support-thread ${expanded === item.id ? "expanded" : ""}`} key={item.id}><button className="support-thread-summary" onClick={() => toggleTicket(item.id)}><div><small>{item.ticket_number} · {item.category}</small><h3>{item.subject}</h3><span>Updated {new Date(item.updated_at).toLocaleString()}</span></div><span className={`ticket-status ${item.status}`}>{statusLabel[item.status] || item.status}</span><ChevronDown /></button>{expanded === item.id && <div className="support-thread-body"><div className="support-messages">{(messages[item.id] || []).map((entry) => <div className={`support-message ${entry.author_role}`} key={entry.id}><b>{entry.author_role === "admin" ? "GoldOnTheSpot Support" : "You"}</b><p>{entry.message}</p><small>{new Date(entry.created_at).toLocaleString()}</small></div>)}</div><label className="support-reply-box">{replyReopens ? "Reply and reopen this ticket" : "Reply"}<textarea rows="4" maxLength="5000" placeholder={replyReopens ? "Write your reply to reopen this conversation…" : "Add a message to this conversation…"} value={replies[item.id] || ""} onChange={(e) => setReplies((current) => ({ ...current, [item.id]: e.target.value }))} /></label><div className="support-thread-actions"><button className="button button-gold" onClick={() => runAction(item, "reply")} disabled={busy || !(replies[item.id] || "").trim()}><Send /> {busy === `reply-${item.id}` ? "Sending…" : replyReopens ? "Reply & reopen" : "Send reply"}</button>{!replyReopens && <button className="button button-outline" onClick={() => runAction(item, "close")} disabled={busy}><XCircle /> Close ticket</button>}</div></div>}</article>; })}</div> : <div className="empty-state compact"><h3>No support conversations yet</h3><p>Use the form above when you need help. Your replies will stay organized here.</p></div>}</div></section>}
  </>;
}
